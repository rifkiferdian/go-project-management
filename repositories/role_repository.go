...
func (r *RoleRepository) GetAll() ([]models.Role, error) {
	rows, err := r.DB.Query(`
		SELECT 
			...
			COUNT(DISTINCT CASE WHEN u.deleted_at IS NULL THEN mhr.model_id END) AS user_count,
			r.updated_at
		FROM roles r
		LEFT JOIN role_has_permissions rhp ON rhp.role_id = r.id
		LEFT JOIN model_has_roles mhr ON mhr.role_id = r.id AND mhr.model_type = ?
		LEFT JOIN users u ON u.id = mhr.model_id
		GROUP BY r.id, r.name, r.guard_name, r.updated_at
...
func (r *RoleRepository) GetByID(id int) (*models.RoleDetail, error) {
	...
	if err := r.DB.QueryRow(`SELECT id, name, guard_name FROM roles WHERE id = ?`, id).
		Scan(&role.ID, &role.Name, &role.GuardName); err != nil {
		return nil, err
	}
...
func (r *RoleRepository) UpdateRoleWithPermissions(params RoleUpdateParams) error {
	...
	if _, err := tx.Exec(`
		UPDATE roles 
		SET name = ?, guard_name = ?, updated_at = NOW()
		WHERE id = ?
	`, params.Name, params.GuardName, params.ID); err != nil {
		tx.Rollback()
		return err
	}
